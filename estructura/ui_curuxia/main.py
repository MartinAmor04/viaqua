...
import pandas as pd
from sql_queries import get_data
...
with col2:
    st.image("img/favicon.png", width=150)
...
with st.container():
    ...
    for idx, row in enumerate(get_data()):
        ...
        with col3:
            st.text(row["place"])
        with col4:
            st.audio("./img/sample-3s.wav", format="audio/wav", loop=False)
        with col5:
            if edit_id == row['machine_id']:
                new_value = st.selectbox("Tipo avería", tipos_averia, key=f"select_{row['machine_id']}")
            else:
                st.text(row["alert_type"])
        with col6:
            if edit_id == row['machine_id']:
                if st.button("Guardar", key=f"save_{row['machine_id']}"):
                    st.session_state.alertas.at[idx, "alert_type"] = new_value
                    st.session_state.edit_id = None
                    st.rerun()
            else:
                if st.button("Editar", key=f"edit_{row['machine_id']}"):
                    st.session_state.edit_id = row['machine_id']
                    st.rerun()
